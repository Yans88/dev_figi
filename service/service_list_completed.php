...
if ($_do == 'export'){
    export_service_request_status(COMPLETED);
}

$_limit = RECORD_PER_PAGE;
$_start = 0;
$total_item = count_service_request_by_status(COMPLETED);
$total_page = ceil($total_item/$_limit);
if ($_page > $total_page) $_page = 1;
if ($_page > 0) $_start = ($_page-1) * $_limit;

$data = get_service_request_by_status(COMPLETED, $_start, $_limit);
?>
...
DATA;
    ...
    echo make_paging($_page, $total_page, './?mod=service&sub=service&act=list&status='.strtolower(COMPLETED).'&page=');
