...
$category_list = get_category_list('SERVICE', $_dept);
if (empty($_cat))
...
if  (isset($_POST['save'])){
    ...
    if ($_tab == 'mobile'){
        if (save_notification_mobiles($_dept, $_cat,  'service', $emails))
            $_msg = 'Mobile numbers has been save!';
    } else {
        if (save_notification_emails($_dept, $_cat,  'service', $emails))
...
function change_tab(id)
{
    ...
    $('#tab').val(id);
    if (id=='mobile')
        load_notification_mobiles('<?php echo $_dept?>', '<?php echo $_cat?>', 'service');
    else
        load_notification_emails('<?php echo $_dept?>', '<?php echo $_cat?>', 'service');
}
...
<!--
<h4>Email Setting for Service Notification</h4>
...
<br/>
Type user's  name who will be notified on service processes:<br/>
