...
function query_service_request_by_status($status, $start = 0, $limit = RECORD_PER_PAGE){
	$dept = defined('USERDEPT') ? USERDEPT : 0;
	$dtf = '%d-%b-%Y %H:%i';
	$query  = "SELECT lr.id_loan, date_format(start_loan, '$dtf') as start_loan, date_format(end_loan, '$dtf') as end_loan, 
			 date_format(request_date, '$dtf') as request_date, 
			 user.full_name as requester, category_name, category_type, quantity, remark, status, purpose,
			 approved_by, approval_date, approval_remark, issued_by, issue_remark,  
			 date_format(issue_date, '$dtf') as issue_date, 
			 date_format(return_date, '$dtf') as return_date,  return_remark, returned_by 
			 FROM loan_request lr 
...
function query_service_rejected_request($start = 0, $limit = RECORD_PER_PAGE){
	$dept = defined('USERDEPT') ? USERDEPT : 0;
	$dtf = '%d-%b-%Y %H:%i';
	$query  = "SELECT lr.id_loan, date_format(start_loan, '$dtf') as start_loan, date_format(end_loan, '$dtf') as end_loan, 
			 date_format(request_date, '$dtf') as request_date, 
			 user.full_name as requester, category_name, category_type, quantity, remark, status, purpose,
			 rejected_by, reject_date, reject_remark, 
			 date_format(reject_date, '$dtf') as reject_date, 
			 (SELECT full_name FROM user WHERE id_user=rejected_by) AS rejected_by_name 
			 FROM loan_request lr 
			 LEFT JOIN user ON requester = user.id_user 
			 LEFT JOIN category ON lr.id_category = category.id_category 
			 LEFT JOIN loan_reject lp ON lp.id_loan = lr.id_loan  
			 WHERE category_type = 'SERVICE' AND status = 'REJECTED' ";
    if (!SUPERADMIN)
        $query .= " AND category.id_department = $dept ";
	$query .= " ORDER BY request_date DESC LIMIT $start, $limit";
	$rs = mysql_query($query);
...
function get_service_request_by_status($status, $start = 0, $limit = RECORD_PER_PAGE){
	$result = array();
	if (REJECTED==$status)
		$rs = query_service_rejected_request($start, $limit);
	else
...
function get_service_request($id = 0){
    ...
    $query  = "SELECT lr.id_loan, date_format(start_loan, $dtf) as start_loan, date_format(approval_date, $tmf) as approval_time, 
             ...
             user_email, user.full_name as requester, category_name, remark, status, category.id_department, 
...
function send_completed_service_request_notification($id = 0){
    ...
    $data['expected_service_datetime'] = $data['start_loan'];
    ...
    if(sms_service_request){
		if ($config['enable_notification_sms'] == 'true'){
			...
			$id_msg = set_notification_message($configuration['global']['sms_sender'], $to,  null, $message, null, 'service', 'sms');
			process_notification($id_msg);
			//writelog('send_completed_service_request_notification(): '. $configuration['global']['sms_sender'] . '|' . $to . '|' . $message);
		}    
	}    
}
...
function export_service_request_status($status){
	...
	if (REJECTED==$status)
		$rs = query_service_rejected_request(0, $total);
	else
    ...
    if ($rs) // && (mysql_num_rows($rs)>0)
        ...
        case 'pending':
            ...
            break;
        case 'rejected':
            echo 'No,Date of Request,Requestor,Reject Date,Category,Remarks'.$crlf;
                while ($rec = mysql_fetch_assoc($rs)){
                    echo "$transaction_prefix$rec[id_loan],$rec[request_date],$rec[requester],";
                    echo "$rec[reject_date],$rec[category_name],$rec[reject_remark]$crlf";
                }
            break;
        case 'returned':
            echo 'No,Date of Request,Requestor,Service Date,Category,Remarks'.$crlf;
...
function send_unapproved_service_request_notification($id = 0)
{
    global $transaction_prefix, $configuration;
    $config = $configuration['service'];
    ...
    if ($config['enable_notification_email'] == 'true'){
        ...
        foreach ($email_rec as $rec)
            $emails[] = $rec['email'];
        if (count($emails) > 0) {
            $message = compose_message('messages/service-request-unapproved.msg', $data);
            $to = array_shift($emails);
            $cc = implode(',', $emails);
            $subject = 'Service Request ('. $request_no . ') by ' . $data['requester'] . ' has been un-approved';
            //SendEmail(SYSTEM_EMAIL, $to, $subject, $message, $cc);
            $id_msg = set_notification_message($configuration['global']['system_email'], $to, $subject, $message, $cc, 'service', 'email');
            process_notification($id_msg);        
        }
	...
	if(sms_service_request){
		if ($config['enable_notification_sms'] == 'true'){
			...
			if (!empty($data['contact_no']) && $check_numb_sms)
				array_unshift($mobiles, $data['contact_no']);
			$to = implode(',', $mobiles);
			$message = compose_message('messages/service-request-unapproved.sms', $data);
			//SendSMS(SMS_SENDER, $to, $message);
			$id_msg = set_notification_message($configuration['global']['sms_sender'], $to,  null, $message, null, 'service', 'sms');
			process_notification($id_msg);
			//writelog('send_approved_service_request_notification(): '. $configuration['global']['sms_sender'] . '|' . $to . '|' . $message);
