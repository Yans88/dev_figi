...
$total_item = count_comparison_location($dept);
...
<form method="post" action="">
  <h2>Modify Location Which Available in Figi</h2>
	<table>
		<tr>
			<td colspan=5>
				<a class="button" href="./?mod=item&sub=comparison_asset&act=modifyasset"> < Modify Asset Number </a>
				<a class="button" href="./?mod=item&sub=comparison_asset&act=modifystatus"> Next > Modify Status </a>
			</td>			
		</tr>
	</table>
	<table id="itemlist" cellpadding=2 cellspacing=0 class="itemlist">
		<tr height=30>
		  <th width=30>No</th>
		  <th width=110>Location In File</th>
		  <th width=100>Location In Figi</th>
		...
		if($total_item == 0){
			...
			while($rec = mysql_fetch_array($rs)){
				...
				echo "
					...
					<td align='center'>".$rec['temporary_location']."</td>
					<td align='center'>".$rec['item_location']."</td>
	...
	<table>
		<tr>
			<td colspan='5' align='center'> 
				<?php echo make_paging($_page, $total_page, './?mod=item&sub=comparison_asset&act=modifylocations&page=');?> 
			</td>
		</tr>
		<tr>
			<td>
			<div style='float:right;'><input type='submit' name='add_item' value='Modify Location' class='button' onclick='return confirm("Are you sure want to Update item(s) selected ?")'></div>
			</td>
		</tr>
	</table>
</form>
...
function get_item_temporaries($dept, $_start = 0, $_limit = 10)
{
    ...
    $query  = "
			SELECT e.serial_no, e.id_department, e.id_owner, e.id_category ,(i.location_name) as item_location, (e.location_name) as temporary_location, 
				(e.id_location) as id_location_itemtemporaries, (i.id_location) as id_location_item 
			FROM 
				(SELECT item_temporaries.*, location_name FROM item_temporaries LEFT JOIN location ON location.id_location = item_temporaries.id_location ) as e, 
				(SELECT item.*,location_name FROM item LEFT JOIN location ON location.id_location = item.id_location) as i, 
				category 
			WHERE 
				i.id_location != e.id_location AND i.serial_no = e.serial_no AND e.id_category = category.id_category";
...
function count_comparison_location($dept, $_start = 0, $_limit = 10)
{
    $query  = "
			SELECT count(e.serial_no) as total, e.id_department, e.id_owner, e.id_category ,(i.location_name) as item_location, (e.location_name) as temporary_location, 
				(e.id_location) as id_location_itemtemporaries, (i.id_location) as id_location_item 
			FROM 
				(SELECT item_temporaries.*, location_name FROM item_temporaries LEFT JOIN location ON location.id_location = item_temporaries.id_location ) as e, 
				(SELECT item.*,location_name FROM item LEFT JOIN location ON location.id_location = item.id_location) as i, 
				category 
			WHERE 
				i.id_location != e.id_location AND i.serial_no = e.serial_no ";
...
function modify_location($serial_no){
	...
	$serial_no = $item_temp['serial_no'];
		...
		$query_update = "UPDATE item SET id_location = '$id_location', status_update = '$date_now' WHERE serial_no = '$serial_no'";
