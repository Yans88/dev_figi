...
if($save == 'save'){	
	if(!empty($import_val)){
		...
		for($i=0; $i<$count_val;$i++){
			$dt = explode(',',$val[$i]);			
			$manufacturer_name = $dt[0];			
			$query = "REPLACE INTO manufacturer (manufacturer_name) VALUES ('$manufacturer_name')";
...
function vendor_import($path)
{
   ...
   while (($data = fgetcsv($handle, 1000, ",",'"')) !== FALSE) {
	   if($row>1){
		   $manufacture_name = htmlspecialchars($data[0], ENT_QUOTES);
		   if(!empty($manufacture_name)){
			   $_data[] = array('manufacture_name' => $manufacture_name);
		   }
	   }    
	$row++;
   }  
...
if(!empty($dt_import)){
	...
	echo '<table width=400 cellpadding=2 cellspacing=1 class="itemlist" >
<tr height=30>
  	<th width=30>No</th>
	<th>Manufacture </th>
</tr>';
...
foreach($dt_import as $_import){
	echo '<tr>';
	echo '<td>'.$i.'</td>';
	echo '<td>'.$_import['manufacture_name'].'</td>';
	echo '</tr>';
	if(!empty($_import['manufacture_name'])){
		$valid_data[] = $_import['manufacture_name'];
	}	
	$i++;
}
...
<tr>
  <td align="center"> <input type="submit" name="import" value=" Import Manufacture(s) " > </td>
</tr> 
<tr><td height=20>&nbsp;</td></tr>
<tr><td align='center'>To download CSV Template, Click <a href="./?mod=item&sub=manufacturer&act=import_template">Here</a></td></tr>  
</table>  
</form>
