...
/**
function get_parent_child($data_arr, $unsetNolMinus=true){
	if($unsetNolMinus){
		foreach($data_arr as $key => $value){
			if($key <1) unset($data_arr[$key]);
		}
	}

	foreach($data_arr as $key => $value){
		$arr=mysql_query("SELECT * FROM category WHERE id_parent=$key");
...
function categoriesTree($id=0) {
	if($id!=0) $where = " WHERE id_parent = '" . $id . "' ";
    $s = "SELECT * FROM category  $where ORDER BY id_parent, id_category";
    $ex =mysql_query($s);
	$rid= mysql_fetch_assoc($ex);
    $treeArray = array();
	//print_r($rid);
    foreach ($rid as $row) {
	//echo $row.'<br>';
        $treeArray[$row['id_category']] = $row;
        if ($row['id_parent'] == 1) {
            $treeArray[$row['category_name']] = categoriesTree($row['id_category']); //results in Fatal error: Maximum function nesting level of '100' reached, aborting!
        }
    }
    return $treeArray;
}
...
function get_parent_category(){
	$result=array();
	$qry="SELECT id_category,category_name FROM category WHERE id_category=id_parent ORDER BY category_name ASC";
	$exec = mysql_query($qry);
	while($r=mysql_fetch_array($exec)){
		//$result['id'][]=$r['id_category'];
		//$result['name'][]=$r['category_name'];
		$result[$r['id_category']]=$r['category_name'];
	}
...
*/
/**
function show_tree($level=4){
	$parent = get_parent_category();
	for($i=1; $i<=$level; $i++){
		$level_field[]="t{$i}.category_name AS lev{$i}";
		if($i>1){
			$before=$i-1;
			$join[]="LEFT JOIN category AS t{$i} ON t{$i}.id_parent = t{$before}.id_category ";
		}
	}
	$result='<select>';
	foreach($parent as $id=>$name){
		$qry = 'SELECT '.implode(',',$level_field).' FROM category AS t1 '.implode(' ',$join).
			   'WHERE t1.id_category ='.$id.' AND t1.category_name != t2.category_name
			   ORDER BY t1.category_name,t2.category_name';

		$exec = mysql_query($qry);
		
		$result.= '<option value='.$id.'>'.$name.'</option>';
		while($r=mysql_fetch_array($exec)){
			if(!in_array($r['lev2'],$parent) && !in_array($r['lev3'],$parent)){
				if($r['lev2']!=null){
					$result.= '<option value='.$r['lev2'].'> '.$r['lev2'].'</option>';
					$result.= '<option value='.$r['lev3'].'> '.$r['lev3'].'</option>';
				}
			}
		}
	}
...
function get_items($orderby = 'asset_no', $sort = 'asc', $start = 0, $limit = 10, $searchby = null, $searchtext = null, $dept = 0, $all = false)
{
    ...
    $fmt = '%d-%b-%Y';
    $query  = "SELECT item.*, status_name, brand_name, category_name, vendor_name, manufacturer_name, department_name, full_name issued_to_name, location_name, 
