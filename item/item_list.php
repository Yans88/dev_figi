...
if ($i_can_create){
	...
	$page_quickII = get_page_privileges(USERGROUP, get_pages_id_by_name('Import Item'));
	$i_can_view_quickII = (isset($page_quickII[CAN_VIEW] ) && ($page_quickII[CAN_VIEW] == 1));
	if($i_can_view_quickII){
		echo '<a class="button" href="./?mod=item&act=import"> Import... </a> ';
	}	
	$page_quickTII = get_page_privileges(USERGROUP, get_pages_id_by_name('Temporary Item Import'));
	$i_can_view_quickTII = (isset($page_quickTII[CAN_VIEW] ) && ($page_quickTII[CAN_VIEW] == 1));
	if(SUPERADMIN && temporary_import_enabled){
		$z = strtolower(TEMPORARY_IMPORT_DEFAULT);
		if($z == "on"){			
			if($i_can_view_quickTII){
				echo '<a class="button" href="./?mod=item&act=import_temporary"> Temp... </a> ';
			}
		} 
	}
    ...
    if ($total_item > 0){
		$page_quickEI = get_page_privileges(USERGROUP, get_pages_id_by_name('Export Item'));	
		...
		if(SUPERADMIN){
			 ...
			 if(QR_CODE) echo '<a class="button" href="./?mod=item&act=qrcode"> Labels</a> ';
			 if ($i_can_create){
...
<div class="searchbox" >
	...
	<?php if (SUPERADMIN){ ?>
    <option value="department_name" <?php if ($_searchby == 'department_name') echo 'selected'?>>Department</option>
	<?php } ?>
	...
	<option value="hostname" <?php if ($_searchby == 'hostname') echo 'selected'?>>Host Name</option>
	<option value="cost" <?php if ($_searchby == 'cost') echo 'selected'?>>Cost</option>
    </select>
...
<?php if (SUPERADMIN || (USERGROUP == GRPASSETADMIN)){ ?>
...
{
	$edit_link = null;
	if ($i_can_update && ($rec['id_status']!=CONDEMNED))
		$edit_link = '<a href="?mod=item&act=edit&id='.$rec['id_item'].'" title="edit"><img class="icon" src="images/edit.png" alt="edit"></a>';
	if ($i_can_delete)
		$edit_link .='<a href="?mod=item&act=del&id='.$rec['id_item'].'" onclick="return confirm(\'Are you sure you want to delete '.$rec['asset_no'].'?\')" title="delete"><img class="icon" src="images/delete.png" alt="delete"></a>';

	
	$dept_name = (USERDEPT > 0) ? null : "	<td>$rec[department_name]</td>";
	$_class = ($counter % 2 == 0) ? 'class="alt"':'class="normal"';
	$dept_col = (SUPERADMIN || (USERGROUP == GRPASSETADMIN)) ? "<td>$rec[department_name]</td>" : '';
	echo <<<DATA
	<tr $_class>
