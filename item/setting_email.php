...
if  (isset($_POST['save'])){
    ...
    if ($_tab == 'mobile'){
        if (save_notification_mobiles($_dept, $_cat,  'item', $emails))
            $_msg = 'Mobile numbers has been save!';
    } else {
        if (save_notification_emails($_dept, $_cat,  'item', $emails))
...
$_emails = '';
/*
$emails = get_notification_emails($_dept, $_cat, 'item');
...
function change_tab(id)
{
    ...
    $('#tab').val(id);
    if (id=='mobile')
        load_notification_mobiles('<?php echo $_dept?>', '<?php echo $_cat?>', 'item');
    else
        load_notification_emails('<?php echo $_dept?>', '<?php echo $_cat?>', 'item');
}
...
<br/>
Type user's  name who will be notified on item issuance:<br/>
