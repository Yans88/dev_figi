...
if (!empty($_SESSION['ITEM_FIELD']))
    ...
    $item_field = array('store_name|Store Type','category_name|Category','cost|Cost of Item','invoice|Invoice Number','status_name|Status') ;
...
$item_f = array('id_category'=>$_category,'id_store'=>$_store,'model_no'=>$model_no,'id_status'=>$id_status,'id_location'=>$_id);
if(SUPERADMIN){
...
$order_link = './?mod=report&sub=item&term=list&by=department&chgord=1&searchby='.$_searchby.'&id_department='.$_searchtext.'&page='.$_page.'&ordby=';
...
<input type="hidden" name="by" value="department">
...
<div style="display: inline-block;text-align: left; float: left; width: 70%;  font-weight:bold" id="filter_group">
	...
	<div>Filtered by Department <?php echo build_combo('id_department', $department, $_dept, 'reload_location(this)');?> </div>
...
<div style="float: right">
    <a class="button" href="./?mod=report&sub=item&term=export&by=department&chgord=1&searchby=id_department&id_department=<?php echo $_searchtext?>&ordby=<?php echo $_orderby?>&id_category=<?php echo $_GET['id_category']?>&id_store=<?php echo $_GET['id_store']?>&model_no=<?php echo $_GET['model_no']?>&id_status=<?php echo $_GET['id_status']?>">Export</a>
</div>
<?php } ?>
</div>
