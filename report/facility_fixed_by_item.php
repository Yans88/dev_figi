...
if ($_view){
	...
	if ($rs && mysql_num_rows($rs) > 0) {
		...
		$query = "select std.id_item, l.id_location, l.location_name, count(st.id_location) as ttal from location l, 
				  students_trans st, students_trans_detail std where l.id_location = st.id_location and std.id_item = ".$_item;
		if (!$_export)
...
<tr><td align="right"><a class="button" href="#" onclick="export_this()">Export</a></td></tr>
<tr><td colspan=2>
...
TABLE;
		...
		if (mysql_num_rows($res)>0){
		  while ($rec=mysql_fetch_array($res)){		
			...
			echo make_paging($_page, $total_page, './?mod=report&sub=facility&act=view&term=fixed&by=item&view=1&username='.$_username.'&page=');
			echo  '</td></tr>';			
		} else
