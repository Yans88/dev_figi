...
<tr>
	...
	<td colspan=7 align="right"><a class="button" href="#" onclick="export_this()">Export</a></td>
</tr>
<tr><th>Category</th>
HEAD;
foreach ($statuses as $id_status => $status_name)
...
foreach ($categories as $id_category => $category_name){ // baris
	...
	foreach ($statuses as $id_status => $status_name) {
        ...
        if ($summary[$id_status][$id_category] > 0)
            echo '<td align="center"><a class="statuslink" href="./?mod=report&sub=item&act=list_by_status&status='.$id_status.'&cat='.$id_category.'">'.$summary[$id_status][$id_category].'</a></td>';
		else
