...
$opt_sel = array('model_no|Model Number','location_name|Location','department_name|Department','store_name|Store Type','category_name|Category','status_name|Status','cost|Item cost','brand_name|Brand name');
...
$base_array = array('asset_no|Asset No','serial_no|Serial Number');
$added_array = $item_field;
$field_export = array_merge($base_array,$item_field);
/////////////////////////////////////////////////////////////////////////
...
$order_link = './?mod=report&sub=item&term=list&by=location&chgord=1&searchby='.$_searchby.'&id_location='.$_searchtext.'&page='.$_page.'&ordby=';
?>
<br/>
...
<input type="hidden" name="by" value="location">
<input type="hidden" name="ordby" value="<?php echo $_orderby?>">
<div class="middle" style="display: inline-block;text-align: left; float: left; width: 70%;  font-weight:bold" id="filter_group">
   
	<div>Filtered by Location <?php echo build_combo('id_location', $locations, $_id, 'reload_location(this)');?> </div>
    <div>Category <?php echo build_combo('id_category', $categories, $_category, 'reload_location(this)');?> </div>
	<div>Store Type <?php echo build_combo('id_store', $store, $_store, 'reload_location(this)');?></div>
	<div>Model Number <input type='text' name="model_no" value='<?php echo $model_no ?>' onblur='reload_location(this)'></div>
	<?php if(SUPERADMIN){?><div>Department<?php echo build_combo('id_department', $department, $_dept, 'reload_location(this)');?></div><?php }?>
...
<div style="display: inline-block;text-align: left; float: left;  font-weight:bold">
	...
	<select multiple id="choose_field" name="choose_field[]" size="3" onblur="reload_location(this)">
		...
		<?php foreach($opt_sel as $row){
			$optex = explode('|',$row);
				$sel_opt = in_array($row,$item_field) ? 'selected' : '';
...
<div style="float: right">
    <a class="button" href="./?mod=report&sub=item&term=export&by=location&chgord=1&searchby=id_location&id_location=<?php echo $_searchtext?>&ordby=<?php echo $_orderby?>&id_department=<?php echo $_GET['id_department']?>&id_category=<?php echo $_GET['id_category']?>&id_store=<?php echo $_GET['id_store']?>&model_no=<?php echo $_GET['model_no']?>&id_status=<?php echo $_GET['id_status']?>&field=<?php echo implode(',',$field_export)?>">Export</a>
</div>
<?php } ?>
</div>
...
echo  '</td></tr></table><br/>';
     ...
     echo '<br>&nbsp;<p class="msg error" style="margin-top: 10px">Data is not available!.</p>';
}
?>
<script>
    $('#searchtext').focus();
