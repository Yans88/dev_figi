...
$summary = array();
$categories = get_fault_category_list($dept);
if (isset($_POST['act'])){
	$query = "SELECT MONTH(report_date) monkey, fault_category, count(id_fault) num_of_faults  
				FROM fault_report fr 
				WHERE YEAR(report_date) = $_year AND fault_status = 'COMPLETED' 
				GROUP BY monkey, fault_category ";
	$res = mysql_query($query);

	while ($rec = mysql_fetch_array($res)) {   
		$summary[$rec['monkey']][$rec['fault_category']] = $rec['num_of_faults'];
	}
}
...
if (isset($_POST['act']) && $_POST['act'] == 'export'){
	...
	header("Content-Disposition: attachment; filename=fault_frequency_by_status.csv");
...
<tr>
	<td colspan=4><h4>In Terms Of Fault Type</h4></td>
	<td colspan=10 align="right"><a href="#" class="button" onclick="export_this()">Export</a></td>
</tr>
<tr><th>Fault Type</th>
RESULT;
