...
function loan_save_request(){
    ...
    //$students_loan = mysql_real_escape_string($_POST['student_loan']);
    $status = 'PENDING'  ;
    //$status = (REQUIRE_LOAN_APPROVAL) ? 'PENDING' : 'APPROVED' ;
    $query = "INSERT INTO loan_request(requester, id_category, start_loan, end_loan, 
				quantity, remark, purpose, request_date, status, without_approval, id_department,students_loan) 
				VALUES ($userid, $_POST[id_category], '$start_date', '$end_date',
				$quantity, '$remark', '$purpose',  now(), '$status', $without_approval, '$_POST[id_department]', '$_POST[student_loan]')"; 
...
$_studentLoan = (!empty($_GET['loan_student'])) ? $_GET['loan_student'] : 0;
...
<div id="tab_loan" class="tabset_content">
    &nbsp;
     ...
     <input type=hidden name=portal value="LOAN">
	  <input type=hidden name=student_loan value="<?php echo $_studentLoan;?>">
...
function fill_purpose(id, thisValue, onclick) {
    $('#'+id).val(thisValue);
    var suggest_for = id.substring(8);
    setTimeout("$('#suggestions_loan').fadeOut();", 100);
}
