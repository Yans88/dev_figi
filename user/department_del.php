...
if ($_id > 0) {
  ...
  if (mysql_num_rows($rs) > 0) {
	  ...
	  $rs = mysql_query($query);
      $_msg = "Department '$data[department_name]' deleted!";
	   user_log(LOG_DELETE, 'Delete department '. $data['department_name']. '(ID:'. $_id.')');
  } else
	$_msg = "Department is not found!";
} else 
	$_msg = "Department's ID is not specified!";
