...
<a class="button" href="./?mod=facility&sub=facility&act=edit&id=<?php echo $_id?>" title="Add new periode">
	<img width=16 height=16 border=0 src="images/back.png"></a>
<a class="button" href="./?mod=facility&sub=timesheet&act=edit&id=<?php echo $_id?>" title="Add new periode">
	<img width=16 height=16 border=0 src="images/add.png"></a>
<a class="button" href="./?mod=facility&sub=timesheet&act=gen&id=<?php echo $_id?>" 
...
<a class="button" href="./?mod=facility&sub=timesheet&act=copy&id=<?php echo $_id?>"  title="Copy time sheet from other facility">
...
if ($item_count > 0){
	$row = 0;
	foreach ($timesheet_data as $rec){
		$row++;
        $period = "$rec[time_start] - $rec[time_end]";
		$class =($row % 2 == 0 ) ? ' class="alt"' : ' class="normal"';
		$link  = '<a href="./?mod=facility&sub=timesheet&act=edit&id='.$rec['id_facility'].'&sid='.$rec['id_time'].'">edit</a> | ';
		$link .= '<a href="./?mod=facility&sub=timesheet&act=del&id='.$rec['id_time'].'"';
        $link .= ' onclick="return confirm(\'Are you sure delete period '.$period.' ?\')">delete</a> ';
		//$link .= '<a href="./?mod=facility&sub=timesheet&act=view&id='.$rec['id_facility'].'">timesheet</a>';
		echo <<<ROW
<tr $class>
