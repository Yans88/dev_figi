...
if  (isset($_POST['save'])){
    ...
    if ($_tab == 'mobile'){
        if (save_notification_mobiles($_dept, 0,  'facility', $emails))
            $_msg = 'Mobile numbers has been save!';
    } else {
        if (save_notification_emails($_dept, 0,  'facility', $emails))
...
/*
$emails = get_notification_emails($_dept, 0, 'facility');
...
function change_tab(id)
{
    ...
    $('#tab').val(id);
    if (id=='mobile')
        load_notification_mobiles('<?php echo $_dept?>', 0, 'facility');
    else
        load_notification_emails('<?php echo $_dept?>', 0, 'facility');
}
...
<!--
<h4>Email Setting for Facility Notification</h4>
...
<br/>
Type user's  name who will be notified on facility booking request:<br/>
