...
if (!empty($_POST['save'])){
	...
	if (mysql_query($query)){
		...
		if ($id_term > 0)
			$_msg = 'Period term information has been saved successfully';
		else
...
if (!empty($_POST['dele'])){
	...
	if (mysql_query($query)){
		...
		if ($ok) $_msg = 'Selected period term has been deleted successfully!';
...
<br>
<table class="tbl_period_term_edit">
...
if ($total_item > 0){
?>
<a href="javascript:add()" class="button" id="addperiodterm">Add Term</a>
...
echo '</td></tr>';
	...
	echo '<tr ><td colspan=8 class="pagination error">Data is not available!. Click <a href="javascript:add()" >here</a> to create new term.</td></tr>';
?>
</table>
<br/>
