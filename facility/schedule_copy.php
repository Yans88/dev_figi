...
if (isset($_POST['copy'])) {
    ...
    if ($_src_id > 0){
        ...
        if (count($schedules) > 0){ // has time sheet
        ...
        $query = "INSERT INTO facility_schedule (id_facility, time_start, time_end) 
                    SELECT $_id, time_start, time_end FROM facility_schedule 
                    WHERE id_facility = $_src_id ";
        mysql_query($query);        
        $_msg = "Copy time sheet from '$facility_data_from[facility_no]' to '$facility_data[facility_no]' is succeed!";
    } else {
