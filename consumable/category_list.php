...
if (!empty($_SESSION['CATEGORY_ORDER_STATUS']))
    ...
    $order_status = array('equipment' => 'asc', 'service' => 'asc', 'consumable' => 'asc');
...
<tr height=30>
  ...
  <th <?php echo $row_class ?>>
	<a href="./?mod=consumable&sub=category&type=<?php echo $type?>&page=<?php echo $_page?>&sort=1">Category</a></th>
...
while ($rec = mysql_fetch_array($rs))
{
  ...
  if (!SUPERADMIN && $i_can_update)
    ...
    <a href="?mod=consumable&sub=category&act=del&id=$rec[id_category]" 
...
echo make_paging($_page, $total_page, './?mod=consumable&sub=category&act=list&type='.$type.'&page=');
echo  '</td></tr>';
    ...
    if (dept != ''){
        $.post("item/category_update.php", {type: ""+cat_type+"", dept: ""+id_dept+"", id: 0, name: ""+dept+""}, function(data){
            ...
            if (data.length>0 && parseInt(data) > 0){
                ...
                location.href = "./?mod=consumable&sub=category";
            } else {
