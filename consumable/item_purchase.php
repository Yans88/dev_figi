...
if ($_input != null){
    $rec = get_consumable_item_by_code($_input);
    if (count($rec) > 0)
        $_id = $rec['id_item'];
}

if (isset($_POST['save']) && ($_POST['save'] == 1) && ($_id > 0) ) {
	$_id = purchase_consumable_item($_id, $_POST);
	if ($_id > 0){
		echo '<script>alert("Purchased item updated");location.href="./?mod=consumable&act=purchase&id='.$_id.'"</script>';
		return;
	} else {
