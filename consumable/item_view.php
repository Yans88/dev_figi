...
if (count($data_item) == 0){
    echo '<script>location.href = "./?mod=consumable&sub=item";</script>';
    return;
}
...
function suggest(me, inputString){
	if(inputString.length == 0) {
		...
		$.post("consumable/suggest_author.php", {queryString: ""+inputString+"", inputId: ""+me.id+""}, function(data){
			if(data.length >0) {
				$('#suggestions').fadeIn();
...
function edit_it()
{
    location.href="./?mod=consumable&act=edit&id=<?php echo $_id?>";
}

function purchase(id)
{
    location.href="./?mod=consumable&act=purchase&id="+id;
}
