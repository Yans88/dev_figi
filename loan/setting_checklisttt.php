...
if  (isset($_POST['save'])){
	if ($_id>0){
		$query = "UPDATE loan_out_checklist SET title='$_title',id_category=$_cat, is_mandatory=$_mandatory, is_enabled=$_enabled WHERE id_check=$_id";
		if (mysql_query($query) && mysql_affected_rows()>0)
			$_msg = 'Title has been updated!';
		else
			$_msg = 'Title failed to update!';
	} else {
		$query = "INSERT loan_out_checklist(title, is_mandatory, id_category, id_department, is_enabled) VALUE('$_title', $_mandatory, $_cat, $_dept, $_enabled)";
		if (mysql_query($query) && mysql_affected_rows()>0)
...
if (($rs=mysql_query($query))&&mysql_num_rows($rs)>0){
	...
	$the_list .= "<tr><th width=20>No</th><th width=150>Title</th><th width=120>Category</th><th width=100>Type of status</th><th width=70>Status</th></tr>";
	$no = 1;
	while ($rec = mysql_fetch_assoc($rs)){
		$is_mandatory = ($rec['is_mandatory']) ? 'Mandatory' : 'Optional';
		$is_enabled = ($rec['is_enabled']) ? 'Enabled' : 'Disabled';
		$cn = ($no % 2 == 0) ? 'alt' : '';
		$the_list .= "<tr class='itemrow $cn' id='row-$rec[id_check]'><td>$no</td><td class='$rec[title] ct'>$rec[title]<input type='text' class='ttl' name='ttl[]' style='display:none;' value='$rec[title]'></td><td>$rec_category[category_name]</td><td class='cm'>$is_mandatory</td><td class='ce'>$is_enabled</td></tr>";
		$no++;
	}
	$the_list .= '</table>';
} else
...
<input type="text" id="id_check" name="id_check" value="" style="display:none;">
...
<input type="checkbox" name="is_enabled" id="is_enabled" value=1 <?php echo ($_enabled>0) ? 'checked' : null?>>Enabled
</fieldset>
<fieldset class="footer" id="btnset">
<button type="reset" id="reset"> Reset </button>
<button type="button" id="save_button"> Save Title </button>
<!--
<button type="button" id="try">Check</button>
-->
</fieldset>
</form>
<br/>
