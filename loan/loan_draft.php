...
$item_onloan = check_status_item($assetNo);
//print_r($test);
if(!empty($item_onloan)){
	?>
	<div id="dialog_item" class="dialog">
	
	<table width="100%" class="grid itemlist">
	<tr>
	...
	<th>Model</th>
		...
		<?php 
		$asset_no_available = implode(',',$item_onloan['id_item']);
		echo '<input type="text" id="asset_no_available" value="'.$asset_no_available.'"/>';
		foreach($assetNo as $asset){
			echo '<tr class = an_item>';			
			echo '<td>'.$item_onloan['asset_no'][$asset].'</td>';
			echo '<td>'.$item_onloan['category'][$asset].'</td>';
			echo '<td>'.$item_onloan['brand'][$asset].'</td>';
			echo '<td>'.$item_onloan['model_no'][$asset].'</td>';
			echo '</tr>';
...
<tr>
    ...
    <tbody id="loan_issuance">
      <tr valign="top">  
        ...
        <td align="left" width=500 style=""><?php echo $issue['name']?>
		<!--
        ...
        <td align="left" colspan=2>Projected Date to return</td>
        </tr>  
      ...
      <tr valign="top" class="alt">  
        ...
        <td align="left" colspan=3> <?php echo $request['department_name']?> </td>    
      </tr>  
      <tr valign="top">  
...
<tr valign="middle">
    <th width="25%">&nbsp;</th>
    <th width="25%" align="center">Approved By</th>
...
<table cellpadding=2 cellspacing=1 width="100%" class="request issue itemlist">
	<tr>
		...
		<td align="center"><button id="btn_draft" class="button">Save as Draft</button>&nbsp;&nbsp;<button id="btn_submit" class="button">Submit</button></td>
	</tr>
</table>
...
function continue_item(){
	var asset = $('#asset_no_available').val();
	var id_loan = '<?php echo $_id;?>';
	if(asset.length > 0){
		$.post("loan/del_item_available.php", {asset: ""+asset+"", loan: ""+id_loan+""}, function(data){
			//redirect("./?mod=loan&sub=loan&act=draft&id=$id_loan");
			location.href = '?mod=loan&sub=loan&act=draft&id='+id_loan;	
		});
	}
} 
