...
$id_loan = $_GET['id'] ? $_GET['id'] : 0;
$_condition = $_GET['condition'] ? $_GET['condition'] : 0;
$condition = explode("|",$_condition);
...
if (isset($_POST['returning']) && ($_POST['returning'] == 1)){
	...
	foreach ($_POST['item_location'] as $id_item => $item_location)
        $item_locations[$id_item] = $item_location;
    ...
    if ($no_of_loaned_items>0){
        foreach ($item_ids as $id)
      	...
      	$_location = $item_locations[$id];
        update_status_returned_items($id_loan, $return_date, $available_items, $return_no, AVAILABLE_FOR_LOAN, $_location);
        update_status_returned_items($id_loan, $return_date, $storage_items, $return_no, STORAGE, $_location);
        update_status_returned_items($id_loan, $return_date, $faulty_items, $return_no, FAULTY, $_location, 'Faulty on loan #'.$id_loan);
        update_status_returned_items($id_loan, $return_date, $lost_items, $return_no, LOST, $_location, 'Lost on loan #'.$id_loan);
        
		//INSERT THE CONDITION HERE
		$x = insert_condition($id_loan, $admin_id, $return_date, $condition1, $condition2, $condition3, $reason);
	...
	$remark = mysql_real_escape_string($_POST['return_remark']) ? mysql_real_escape_string($_POST['return_remark']) : "-NIL-";
...
?>
		...
		<tr valign="top">
        <th colspan=4>Item Returned Condition</th>
        </tr>
		 <tr valign="top">
        <td colspan=4>
			<ul>
				<?php
			if(! empty($condition[0])){ echo "<input type='hidden' name='condition1' value='".$condition[0]."'><li>".$condition[0]."</li> "; } else { 
				echo "<input type='hidden' name='condition1' value=''>";
			}
			if(! empty($condition[1])){ echo "<input type='hidden' name='condition2' value='".$condition[1]."'> <li>".$condition[1]."</li>"; } else { 
				echo "<input type='hidden' name='condition2' value=''>";
			}
			if(! empty($condition[3])){ echo "<input type='hidden' name='reason_txt' value='".$condition[3]."'>"; $reason = $condition[3]; } else {
				echo "<input type='hidden' name='reason_txt' value=''>"; $reason = "";
			}
			if(! empty($condition[2])){ 
				echo "<input type='hidden' name='condition3' value='".$condition[2]."'><li>".$condition[2]." :".$reason."</li>"; } else { 
				echo "<input type='hidden' name='condition3' value=''>";
			}
...
<tr>
    <td align="right" valign="middle" colspan=2>
		<div style="float: left; padding: 5px 10px;">
			<a href="javascript: open_scan()" class="button" id="btn_scan" style="padding: 15px 15px; font-size: 12pt; margin-top: 5px;cursor: pointer "> Scan Item </a>
		</div>
		<div align="right">
			<button type="button" id="btn_submit" style="padding: 2px 15px; font-size: 11pt; margin-top: 4px;cursor: pointer; margin-right: 10px "> Submit </button>
		</div>
    </td>
</tr>
</table>
...
<div id="scan_item" class="dialog ui-helper-hidden">
	<div style="text-align: center">
		<p>Scan items' asset no ...</p>
		<p><input type="text" class="scan" name="scan_asset" id="scan_asset" ></p>
		<p><ol class="asset_list" id="di_msg"></ol></p>
...
<div id="scan_user" class="dialog ui-helper-hidden">
	<div style="text-align: center">
		<p>Scan NRIC ...</p>
		<p><input type="text" class="scan" name="scan_nric" id="scan_nric" ></p>
		<p id="dn_msg"></p>
...
<div id="scan_condition" class="dialog ui-helper-hidden">
	<div style="text-align: center">
		
		<p><input type="checkbox" class="scan" name="condition" id="damaged" value="Damaged"> Damaged</p>
		<p><input type="checkbox" class="scan" name="condition" id="lost_acessories" value="Lost Acessories"> Lost Acessories</p>
		<p><input type="checkbox" class="scan" name="condition" id="other_with_remark_box" value="Others"> Others</p>
		<p><textarea style="resize:none;width:200px;height:100px;font-size:12px;font-family:arial;" name="condition_reason" id="condition_reason" maxlength="100"></textarea></p>
...
<form method="post" id="frm_spec" style="display:none;">
	<input type="text" name="asset_item_txt" id="asset_txt">
	<input type="text" name="nric_item_txt" id="nric_txt">
	<input type="checkbox" name="damaged_txt" id="damaged_txt" value="Damaged">
	<input type="checkbox" name="lost_acessories_txt" id="lost_acessories_txt" value="Lost Acessories">
	<input type="checkbox" name="other_with_remark_box_txt" id="other_with_remark_box_txt" value="Others">
	<input type="text" name="reason_txt" id="reason_txt">
	<input type="submit" name="submit"  id="submit">         
</form>

<?php
	if($_POST['submit']){
		$asset = $_POST['asset_item_txt'];
		$nric = $_POST['nric_item_txt'];
		
		$damage = htmlspecialchars($_POST['damaged_txt'], ENT_QUOTES);
		$lost_acessories = htmlspecialchars($_POST['lost_acessories_txt'], ENT_QUOTES);
		$other_with_remark_box = htmlspecialchars($_POST['other_with_remark_box_txt'], ENT_QUOTES);
		$reason = htmlspecialchars($_POST['reason_txt'], ENT_QUOTES);
		
		$rs = get_specification_loan_request($asset, $nric);
		if($rs == 0){
			echo "<script>alert('pls re-scan NRIC No. instead of saying Data Not Found.')</script>";
		} else {
			header("location: ./?mod=loan&sub=quick_return&id=".$rs."&condition=".$damage."|".$lost_acessories."|".$other_with_remark_box."|".$reason);
		}
	}
?>
...
var nric_len = <?php  echo NRIC_LENGTH?>;
	...
	var scan_item =
		$('#scan_item').dialog({
			...
			autoOpen: false, 
			buttons:  {
				'Scan Item > Next': function(){
					get_data( $("#scan_asset").val());
				},
				'Close': function(){
					window.location.href='./?mod=loan';
				}
	...
	var scan_user = $('#scan_user').dialog({
		modal:true,
		width:400,
		buttons: {
			'Check Condition > Next' : function(){
				check_loan($("#scan_nric").val(), $("#scan_asset").val());
	...
	$("#condition_reason").hide();
	$("#other_with_remark_box").click(function(){
		if($(this).is(":checked")){
			$("#condition_reason").show();
		} else {
			$("#condition_reason").hide();
		}
	...
	var scan_condition = $('#scan_condition').dialog(function(){
		...
		buttons: {
			'Done' : function(){
				scan_condition.dialog("close");
				
				if( $("#damaged").is(':checked') ){
					$("#damaged_txt").attr('checked', true);
				} else { $("#damaged_txt").attr('checked', false);}
				
				if( $("#lost_acessories").is(':checked') ){
					$("#lost_acessories_txt").attr('checked', true);
				} else {$("#lost_acessories_txt").attr('checked', false);}
				
				if( $("#other_with_remark_box").is(':checked') ){
					$("#other_with_remark_box_txt").attr('checked', true);
					$("#reason_txt").val( $("#condition_reason").val() )
				} else {
					$("#other_with_remark_box_txt").attr('checked', false);
				}
			
				
				scan_user.dialog("close");
				$('#submit').click();
			},
...
function get_data(asset){
	$.post("loan/check_item.php", {asset: ""+asset+""}, function(data){
		if(data.length >0) {
			var asset = JSON.parse(data);
			$("#asset_txt").val(asset);
			scan_item.dialog("close");
			scan_user.dialog("open");
			
        } else {
			alert("Asset No. not found in Loan Records");
			scan_item.dialog("open");
		}
	});
}

function check_loan(nric, asset){
	$.post("loan/check_loan_record.php", {asset: ""+asset+"", nric: ""+nric+""}, function(data){
		if(data.length >0) {
			var id_loan = JSON.parse(data);
			$("#nric_txt").val(nric);
			
			scan_condition.dialog("open");
			
        } else {
			alert("Please re-scan NRIC No. instead of saying Data Not Found.");
		}
	});
}
