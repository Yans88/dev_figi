...
if ($asset_no != null){
    $query = "SELECT li.id_loan, li.id_item, i.asset_no, status, lr.requester, u.full_name, u.nric 
			...
			WHERE category.category_type = 'EQUIPMENT' AND status IN ( 'LOANED', 'PARTIAL_IN') AND i.asset_no = '".htmlspecialchars($asset_no, ENT_QUOTES)."'";
    $rs = mysql_query($query);
    
    if ($rs && (mysql_num_rows($rs) > 0)){
        $drs = mysql_fetch_assoc($rs);
		echo json_encode($drs['asset_no']);
    }
}
echo '';
