...
if  (isset($_POST['save'])){
    ...
    if ($_tab == 'mobile'){
        if (save_notification_mobiles($_dept, $_cat,  'loan', $emails))
            $_msg = 'Mobile numbers has been save!';
    } else {
        if (save_notification_emails($_dept, $_cat,  'loan', $emails))
...
$_emails = '';
/*
$emails = get_notification_emails($_dept, $_cat, 'loan');
...
function change_tab(id)
{
    ...
    $('#tab').val(id);
    if (id=='mobile')
        load_notification_mobiles('<?php echo $_dept?>', '<?php echo $_cat?>', 'loan');
    else
        load_notification_emails('<?php echo $_dept?>', '<?php echo $_cat?>', 'loan');
}
...
Type user's  name who will be notified on loan processes:<br/>
    <input type="text" id="edit_email" name="edit_email" onKeyUp="suggest(this, this.value);" onBlur="fill('edit_email', this.value);" >
