...
DATA;
        ...
        if ($need_approval && $i_can_delete){
            echo ' <a href="./?mod=expendable&sub=loan&act=approve&id='.$rec['id_loan'].'" title="approve"><img class="icon" src="images/ok.png" alt="approve"></a>
                    <a href="./?mod=expendable&sub=loan&act=unapprove&id='.$rec['id_loan'].'" )" title="not approve"><img class="icon" src="images/no.png" alt="not approve"></a>';
        } else if ($i_can_update) {
            if ((!$need_approval && ($rec['status'] == PENDING)) || ($need_approval && ($rec['status'] == APPROVED))){
            
                if (!$need_approval)
                    echo '<a href="./?mod=expendable&sub=loan&act=unapprove&id='.$rec['id_loan'].'" title="not approve"><img class="icon" src="images/no.png" alt="not approve"></a>';
                echo ' <a href="./?mod=expendable&sub=loan&act=issue&id='.$rec['id_loan'].'" title="manage"><img class="icon" src="images/process.png" alt="manage"></a> ';            
            }
        }
    echo '</td></tr>';
  $counter++;
