...
if (!empty($_SESSION['EXPENDABLE_ITEM_ORDER_STATUS']))
    $order_status = unserialize($_SESSION['EXPENDABLE_ITEM_ORDER_STATUS']);
else
...
$total_item = count_expendable_item($_searchby, $_searchtext, $dept);
...
ob_clean();
$_SESSION['EXPENDABLE_ITEM_ORDER_STATUS'] = serialize($order_status);
echo $buffer;
$row_class = ' class="sort_'.$sort_order.'"';
$order_link = './?mod=expendable&sub=item&act=list&chgord=1&searchby='.$_searchby.'&searchtext='.$_searchtext.'&page='.$_page.'&ordby=';
...
function suggest(me, inputString){
	...
	if(inputString.length == 0) {
		...
		$.post("expendable/suggest_item.php", {queryString: ""+inputString+"", inputId: ""+me.id+"", dept: ""+department+"", searchBy: ""+$('#searchby').val()+""}, function(data){
			if(data.length >0) {
				$('#suggestions').fadeIn();
...
<form method="get">
<input type="hidden" name="mod" value="expendable">
...
$rs = get_expendable_items($_orderby, $sort_order, $_start, $_limit, $_searchby, $_searchtext, $dept);
$counter = $_start+1;
while ($rec = mysql_fetch_array($rs))
{
	...
	if (!SUPERADMIN && $i_can_update && $i_can_delete)
		$edit_link = <<<EDIT
<a href="./?mod=expendable&act=edit&id=$rec[id_item]" item_name="edit"><img class="icon" src="images/edit.png" alt="edit"></a>
<a href="./?mod=expendable&act=del&id=$rec[id_item]" 
...
EDIT;
	...
	$avail = get_available_item($rec['id_item']);
	echo <<<DATA
	<tr $_class>
	...
	<a href="./?mod=expendable&act=view&id=$rec[id_item]" item_name="view"><img class="icon" src="images/loupe.png" alt="view" ></a>
	$edit_link
	</td>
	</tr>
DATA;
...
echo make_paging($_page, $total_page, './?mod=expendable&sub=item&act=list&searchby='.$_searchby.'&searchtext='.$_searchtext.'&page=');
echo  '</td></tr></table><br/>';
