...
$total_item = count_expendable_item(null, null, $dept);

if (isset($_POST['export']) ) {
    $dept_name = preg_replace('/[\)\(]/', '', $department_list[$dept]);
    $dept_name = preg_replace('/[\/]/', '', $dept_name);
    $fname = 'expendable-item-' . $dept_name . '.csv';
    $path = TMPDIR .'/'. session_id().'-'.$fname;
    export_csv_expendable_item($path, $dept);
...
<button type="button" onclick="location.href='./?mod=consumable';">Cancel</button>  
</form>
  </td>
</tr>
</table>
