...
if ($_input != null){
    $rec = get_expendable_item_by_code($_input);
    if (count($rec) > 0)
        $_id = $rec['id_item'];
}

if (isset($_POST['save']) && ($_POST['save'] == 1) && ($_id > 0) ) {
	$_id = purchase_expendable_item($_id, $_POST);
	if ($_id > 0){
		echo '<script>alert("Purchased item updated");location.href="./?mod=expendable&act=purchase&id='.$_id.'"</script>';
		return;
	} else {
...
<tr><th colspan=2>Purchase Item</th></tr>
<?php
    if ($_id == 0){
?>
<tr><td colspan=2>
...
<tr valign="top">
    <td width=420>
      ...
      <tr class="normal">
        <td>Quantity</td>
        <td><input type="text" name="quantity" value="<?php echo $_qty?>" size=6></td>
      </tr>
      <tr class="alt">
        <td>Unit Price</td>
        <td>
    ...
    ?>
            <input type="text" name="price"  value="<?php echo $data_item['price']?>" size=6>
        <?php 
    ...
    ?>
       ...
       <tr class="normal">
        <td>Vendor</td>
        <td><?php echo build_vendor_combo(null, $dept)?></td>
      </tr>
      <tr class="alt">
        <td>DO No.</td>
        <td><input type="text" name="do_no" value="" ></td>
      </tr>
 </table>
        </td>
  </tr>
