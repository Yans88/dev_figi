...
$_start = 0;
$total_item = count_condemned_by_status(RECOMMENDED);
$total_page = ceil($total_item/$_limit);
if ($_page > $total_page) $_page = 1;
if ($_page > 0)	$_start = ($_page-1) * $_limit;

$data = get_condemned_issue_by_status(RECOMMENDED, $_start, $_limit);

if (REQUIRE_CONDEMNED_APPROVAL){
    if (defined('ENABLE_SECOND_RECOMMENDATION') && ENABLE_SECOND_RECOMMENDATION)
        $caption = 'Condemned Issue Pending (Director Recommendation)';
    else
...
DATA;
        ...
        if (REQUIRE_CONDEMNED_APPROVAL && $i_can_delete){
            if ((defined('ENABLE_SECOND_RECOMMENDATION') && ENABLE_SECOND_RECOMMENDATION) && USERGROUP==GRPDIR){
                echo ' <a href="./?mod=condemned&sub=condemned&act=recommend2&id='.$rec['id_issue'].'" title="recommended by director"><img class="icon" src="images/ok.png" alt="recommend"></a>';
                echo '<a href="./?mod=condemned&sub=condemned&act=reject&id='.$rec['id_issue'].'" )" title="not approve"><img class="icon" src="images/no.png" alt="not approve"></a>';
            } else if ((USERGROUP==GRPPRI) && (defined('ENABLE_SECOND_RECOMMENDATION') && !ENABLE_SECOND_RECOMMENDATION)){
        ...
        } else if (!REQUIRE_CONDEMNED_APPROVAL && $i_can_update) {
