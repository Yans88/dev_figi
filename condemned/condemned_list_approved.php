...
$_start = 0;
$total_item = count_condemned_by_status(APPROVED);
...
$data = get_condemned_issue_by_status(APPROVED, $_start, $_limit);

?>

<br/>
<h3>Approved Condemnation (In-Process)</h3>
...
DATA;
        if ($i_can_update) {
            echo ' <a href="./?mod=condemned&sub=condemned&act=condemn&id='.$rec['id_issue'].'" title="condemn"><img class="icon" src="images/process.png" alt="manage"></a> ';            
        }
        echo '</td></tr>';
    ...
    echo make_paging($_page, $total_page, './?mod=condemned&sub=condemned&act=list&status='.strtolower(APPROVED).'&page=');
    echo  '</td></tr>';
