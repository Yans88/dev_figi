...
$_start = 0;
$total_item = count_condemned_by_status('REJECTED');
$total_page = ceil($total_item/$_limit);
if ($_page > $total_page)     $_page = 1;
if ($_page > 0)	$_start = ($_page-1) * $_limit;

$data = get_condemned_issue_by_status('REJECTED', $_start, $_limit);

?>
<br/>
<h3>Rejected Condemned Issue</h3>
...
DATA;
        if (REQUIRE_CONDEMNED_APPROVAL && ($i_can_delete)){
           ...
           //         <a href="./?mod=condemned&sub=condemned&act=unapprove&id='.$rec['id_issue'].'" )" title="not approve"><img class="icon" src="images/no.png" alt="not approve"></a>';
        } else if (!REQUIRE_CONDEMNED_APPROVAL && $i_can_update) {
           // echo ' <a href="./?mod=condemned&sub=condemned&act=issue&id='.$rec['id_issue'].'" title="manage"><img class="icon" src="images/process.png" alt="manage"></a> ';            
        }
    echo '</td></tr>';
  $counter++;
    } 
    echo '<tr ><td colspan=9 class="pagination">';
    echo make_paging($_page, $total_page, './?mod=condemned&sub=condemned&act=list&status='.strtolower('REJECTED').'&page=');
    echo  '</td></tr>';
