...
if ($_confirm){
    $query = "INSERT INTO key_loan (loan_start, id_user)
                VALUES (now(), $_id_user)";
    mysql_query($query);
    if (mysql_affected_rows()>0){
        ...
        if (count($values) > 0){
            $query  = 'INSERT INTO key_loan_item (id_loan, id_item) ';
            $query .= 'VALUES ' . implode(',', $values);
            mysql_query($query);
            //echo mysql_error();
        }
        // change loaned item's status 
        $query  = "UPDATE key_item SET status = 'On Loan' ";
...
if (($_nric == null) && ($_input !=null)){ // scan nric
    ...
    $item = get_id_key_by_serial($_input, 'Available for Loan');
...
if (count($items) > 0){
    ...
    if ($rs && mysql_num_rows($rs)>0){
        ...
        while ($rec = mysql_fetch_assoc($rs)){
            ...
            $scanned_list .= '<tr><td align="center">' . ($no++) . '.</td><td>' .$rec['serial_no'] . '</td><td>' .$rec['description'] . '</td><td>'.$dellink . '</td></tr>';
        }                
...
<?php
    if ($_confirm){
        ...
        if ($_id_user != 0) {
?>
    <div id="cmdlabel">Welcome <?php echo $_full_name?>,<br/><p></p>
<?php
    echo $scanned_list;
    if (count($items) == 0)
        echo 'You may scan your key now';
    else
        echo 'Scan another key';
?>
    <br/>    
    </div>
