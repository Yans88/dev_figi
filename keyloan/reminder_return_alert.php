...
$config_hour = $config['return_hours'];
...
if ($rs && mysql_num_rows($rs) > 0){
    ...
    while ($rec = mysql_fetch_assoc($rs)){
	   ...
	   //send_due_date_alert($rec['id_loan'], $rec['diff_hours']);
...
function return_reminder_query()
{
    ...
    $dtf = '%d-%b-%Y %H:%i';
    $query  = "SELECT * FROM ( SELECT kl.id_loan,  TIMESTAMPDIFF(HOUR, now(), loan_start) AS diff_hours
                FROM key_loan kl 
