...
$total_key = count_key_item(null, null, $dept);

if (isset($_POST['export']) ) {
    $dept_name = preg_replace('/[\)\(]/', '', $department_list[$dept]);
    $fname = 'key-item-' . $dept_name . '.csv';
    $path = TMPDIR .'/'. session_id().'-'.$fname;
    export_csv_key_item($path, $dept);
...
<tr><th style="color: white">Export Key Item into CSV File</th></tr>
...
if ($total_key > 0){
    if ($total_key == 1)
        ...
        echo '<br/>There are ' . $total_key . ' items. <br/>Click button "Export Items" to download as CSV file.<br/>';
} else
