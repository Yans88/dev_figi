...
/*UPDATE*/function query_service_request_by_statuses($status){	$dept = defined('USERDEPT') ? USERDEPT : 0;	$query  = "SELECT lr.id_loan, date_format(start_loan, '%d-%b-%Y %H:%i') as start_loan, date_format(end_loan, '%d-%b-%Y') as end_loan, 			 date_format(request_date, '%d-%b-%Y %H:%i') as request_date, 			 user.full_name as requester, category_name, category_type, quantity, remark, status, 			 approved_by, approval_date, approval_remark, issued_by, issue_date, issue_remark, returned_by, 			 date_format(return_date, '%d-%b-%Y') as return_date, 			 return_remark, received_by, receive_date, receive_remark, acknowledged_by, acknowledge_date, acknowledge_remark 	 			 FROM loan_request lr 			 LEFT JOIN user ON requester = user.id_user 			 LEFT JOIN category ON lr.id_category = category.id_category 			 LEFT JOIN loan_process lp ON lp.id_loan = lr.id_loan  			 WHERE category_type = 'SERVICE' ";    if (!SUPERADMIN)        $query .= " AND category.id_department = $dept ";	if ($status != '')		$query .= " AND status = '$status' ";	$query .= " ORDER BY request_date DESC ";	error_log($query.mysql_error());		$rs = mysql_query($query);	//echo $query;	return $rs;}function get_service_request_by_statuses($status){	$result = array();	$rs = query_service_request_by_statuses($status);	if ($rs && (mysql_num_rows($rs)>0))		while ($rec = mysql_fetch_assoc($rs))			$result[] = $rec;    	return $result;}
